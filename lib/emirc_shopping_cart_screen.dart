...
class _EmirCShoppingCartScreenState extends State<EmirCShoppingCartScreen> {
  ...
  void initState() {
    ...
    if (widget.missingItems != null && widget.missingItems!.isNotEmpty) {
      final normalizedMissing =
          widget.missingItems!.map((e) => e.toLowerCase()).toSet();
  ...
  double get _cartTotal =>
      _cartItems.fold<double>(0, (sum, item) => sum + item.selectedPrice);

  String get _checkoutStoreLabel =>
      _cartItems.isEmpty ? 'Store' : _cartItems.first.selectedStore;
  ...
  Future<void> _checkout() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Purchase Saved! Total: ₺${_cartTotal.toStringAsFixed(0)}',
          ),
